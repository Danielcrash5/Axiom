...
std::filesystem::path
ResolveExecutableDirectory(const std::vector<std::string> &commandLineArgs) {
...
std::string ResolveAssetPath(const std::string &configuredPath,
...
void Application::MainLoop() {
    ...
    while (m_Running) {
...

        {
            ...
            MainUpdate();
            double alpha =
                (Time::GetTime() - m_LastFixedUpdate) / m_FixedUpdateInterval;
            Render(alpha);
            ImGuiRender();
