...
namespace axiom {
    ...
    std::array<std::array<bool, SDL_GAMEPAD_BUTTON_COUNT>, 4> Input::s_CurrentGamepadButtons {};
    std::array<std::array<bool, SDL_GAMEPAD_BUTTON_COUNT>, 4> Input::s_PreviousGamepadButtons {};
    ...
    void Input::Update() {
        ...
        for (const std::shared_ptr<ICustomInputDevice>& device : s_CustomDevices) {
            if (device)
                device->Update();
        }
        ...
        for (size_t device = 0; device < s_Gamepads.size(); ++device) {
            ...
            for (int button = 0; button < SDL_GAMEPAD_BUTTON_COUNT; ++button) {
                s_CurrentGamepadButtons[device][button] =
                    gamepad && SDL_GetGamepadButton(gamepad, static_cast<SDL_GamepadButton>(button));
            }
        }
    }
    ...
    void Input::RefreshJoysticks() {
        ...
        const int openCount = std::min<int>(count, static_cast<int>(s_Joysticks.size()));
        for (int i = 0; i < openCount; ++i) {
            ...
            const int buttonCount = std::max(0, SDL_GetNumJoystickButtons(joystick));
    ...
    void Input::UpdateJoysticks() {
        for (int deviceIndex = 0; deviceIndex < s_JoystickCount; ++deviceIndex) {
            ...
            for (size_t axis = 0; axis < device.RawAxes.size(); ++axis) {
                const int16_t raw = SDL_GetJoystickAxis(device.Handle, static_cast<int>(axis));
                device.RawAxes[axis] = raw;

                const float normalizer = raw < 0
                    ? static_cast<float>(-SDL_JOYSTICK_AXIS_MIN)
                    : static_cast<float>(SDL_JOYSTICK_AXIS_MAX);
                device.Axes[axis] = glm::clamp(static_cast<float>(raw) / normalizer, -1.0f, 1.0f);
            }

            for (size_t button = 0; button < device.CurrentButtons.size(); ++button)
                device.CurrentButtons[button] = SDL_GetJoystickButton(device.Handle, static_cast<int>(button));

            for (size_t hat = 0; hat < device.Hats.size(); ++hat)
                device.Hats[hat] = SDL_GetJoystickHat(device.Handle, static_cast<int>(hat));
        }
    }
    ...
    bool Input::IsGamepadPressed(GamepadButton button, int id) {
        if (id < 0 || static_cast<size_t>(id) >= s_CurrentGamepadButtons.size() ||
    ...
    bool Input::IsGamepadJustPressed(GamepadButton button, int id) {
        ...
        return s_CurrentGamepadButtons[id][button] && !s_PreviousGamepadButtons[id][button];
    }
    ...
    float Input::GetJoystickAxis(JoystickAxis axis, int id, float deadzone) {
        JoystickDevice* joystick = GetJoystick(id);
        if (!joystick || axis < 0 || static_cast<size_t>(axis) >= joystick->Axes.size())
            return 0.0f;
    ...
    int16_t Input::GetJoystickAxisRaw(JoystickAxis axis, int id) {
        JoystickDevice* joystick = GetJoystick(id);
        if (!joystick || axis < 0 || static_cast<size_t>(axis) >= joystick->RawAxes.size())
            return 0;
    ...
    bool Input::IsJoystickButtonPressed(JoystickButton button, int id) {
        JoystickDevice* joystick = GetJoystick(id);
        if (!joystick || button < 0 || static_cast<size_t>(button) >= joystick->CurrentButtons.size())
            return false;
    ...
    bool Input::IsJoystickButtonJustPressed(JoystickButton button, int id) {
        ...
        return joystick->CurrentButtons[button] && !joystick->PreviousButtons[button];
    }
    ...
    JoystickHat Input::GetJoystickHat(int hat, int id) {
        JoystickDevice* joystick = GetJoystick(id);
        if (!joystick || hat < 0 || static_cast<size_t>(hat) >= joystick->Hats.size())
    ...
    bool Input::IsJoystickHatJustPressed(JoystickHat mask, int hat, int id) {
        ...
        return (joystick->Hats[hat] & mask) != 0 && (joystick->PreviousHats[hat] & mask) == 0;
    }
    ...
    bool Input::RumbleJoystick(uint16_t lowFrequency, uint16_t highFrequency, uint32_t durationMs, int id) {
        JoystickDevice* joystick = GetJoystick(id);
        return joystick && SDL_RumbleJoystick(joystick->Handle, lowFrequency, highFrequency, durationMs);
    }

    bool Input::RumbleJoystickTriggers(uint16_t leftTrigger, uint16_t rightTrigger, uint32_t durationMs, int id) {
        JoystickDevice* joystick = GetJoystick(id);
        return joystick && SDL_RumbleJoystickTriggers(joystick->Handle, leftTrigger, rightTrigger, durationMs);
    }

    bool Input::SetJoystickLED(uint8_t red, uint8_t green, uint8_t blue, int id) {
        JoystickDevice* joystick = GetJoystick(id);
        return joystick && SDL_SetJoystickLED(joystick->Handle, red, green, blue);
    }

    int Input::RegisterCustomDevice(std::shared_ptr<ICustomInputDevice> device) {
        if (!device)
            return -1;
    ...
    void Input::ClearCustomDevices() {
        s_CustomDevices.clear();
    }

    bool Input::IsCustomDeviceConnected(int id) {
        return id >= 0 && static_cast<size_t>(id) < s_CustomDevices.size() && s_CustomDevices[id] != nullptr;
    }
    ...
    int Input::GetCustomDeviceCount() {
        return static_cast<int>(std::count_if(s_CustomDevices.begin(), s_CustomDevices.end(),
    ...
    int Input::GetCustomDeviceAxisCount(int id) {
        return IsCustomDeviceConnected(id) ? s_CustomDevices[id]->GetAxisCount() : 0;
    }

    int Input::GetCustomDeviceButtonCount(int id) {
        return IsCustomDeviceConnected(id) ? s_CustomDevices[id]->GetButtonCount() : 0;
    }

    int Input::GetCustomDeviceHatCount(int id) {
        return IsCustomDeviceConnected(id) ? s_CustomDevices[id]->GetHatCount() : 0;
    }

    float Input::GetCustomDeviceAxis(int axis, int id, float deadzone) {
        if (!IsCustomDeviceConnected(id) || axis < 0 || axis >= s_CustomDevices[id]->GetAxisCount())
            return 0.0f;

        return ApplyDeadzone(glm::clamp(s_CustomDevices[id]->GetAxis(axis), -1.0f, 1.0f), deadzone);
    }
    ...
    JoystickHat Input::GetCustomDeviceHat(int hat, int id) {
        if (!IsCustomDeviceConnected(id) || hat < 0 || hat >= s_CustomDevices[id]->GetHatCount())
            return Joystick::HatCentered;
    ...
    bool Input::IsCustomDeviceHatJustPressed(JoystickHat mask, int hat, int id) {
        return IsCustomDeviceConnected(id) &&
               hat >= 0 &&
