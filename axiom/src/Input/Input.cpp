...
void Input::UpdateJoysticks() {
    for (int deviceIndex = 0; deviceIndex < s_JoystickCount; ++deviceIndex) {
        ...
        for (size_t axis = 0; axis < device.RawAxes.size(); ++axis) {
            ...
            device.Axes[axis] =
                glm::clamp(static_cast<float>(raw) / normalizer, -1.0f, 1.0f);
        }

        for (size_t button = 0; button < device.CurrentButtons.size(); ++button)
            device.CurrentButtons[button] =
                SDL_GetJoystickButton(device.Handle, static_cast<int>(button));
...
float Input::GetGamepadAxis(GamepadAxis axis, int id) {
    ...
    const float normalizer = sdlAxis == SDL_GAMEPAD_AXIS_LEFT_TRIGGER ||
...
bool Input::RumbleGamepad(uint16_t lowFrequency, uint16_t highFrequency,
    ...
    SDL_Gamepad *gamepad = GetGamepad(id);
    return gamepad &&
           SDL_RumbleGamepad(gamepad, lowFrequency, highFrequency, durationMs);
}

bool Input::RumbleGamepadTriggers(uint16_t leftTrigger, uint16_t rightTrigger,
                                  uint32_t durationMs, int id) {
    SDL_Gamepad *gamepad = GetGamepad(id);
...
bool Input::GetGamepadSensorData(GamepadSensor sensor, float *data,
    ...
    return gamepad && data && valueCount > 0 &&
           SDL_GetGamepadSensorData(
               gamepad, static_cast<SDL_SensorType>(sensor), data, valueCount);
}
...
bool Input::GetGamepadTouchpadFinger(int touchpad, int finger, bool &down,
    ...
    return SDL_GetGamepadTouchpadFinger(gamepad, touchpad, finger, &down, &x,
                                        &y, &pressure);
}

bool Input::IsJoystickConnected(int id) { return GetJoystick(id) != nullptr; }
...
bool Input::RumbleJoystickTriggers(uint16_t leftTrigger, uint16_t rightTrigger,
                                   uint32_t durationMs, int id) {
    JoystickDevice *joystick = GetJoystick(id);
    return joystick && SDL_RumbleJoystickTriggers(joystick->Handle, leftTrigger,
                                                  rightTrigger, durationMs);
}
...
int Input::GetCustomDeviceCount() {
    return static_cast<int>(
        std::count_if(s_CustomDevices.begin(), s_CustomDevices.end(),
...
int Input::GetCustomDeviceButtonCount(int id) {
    return IsCustomDeviceConnected(id) ? s_CustomDevices[id]->GetButtonCount()
                                       : 0;
}
...
float Input::GetCustomDeviceAxis(int axis, int id, float deadzone) {
    ...
    return ApplyDeadzone(
        glm::clamp(s_CustomDevices[id]->GetAxis(axis), -1.0f, 1.0f), deadzone);
}
