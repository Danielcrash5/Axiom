...
namespace axiom::renderer::rhi::vulkan {
    ...
    void VulkanBackend::submit(CommandList &cmd) {
        ...
        // Synchron/blockierend fuer Phase 2/3 - Frame-Pacing mit mehreren
        // Fences "in flight" kommt erst mit der Swapchain in Phase 8.
    ...
    RHIResult<void> VulkanBackend::attachSurface(void *nativeSurfaceHandle) {
        if (nativeSurfaceHandle == nullptr) {
            return std::unexpected(RHIError::InvalidDescriptor);
        }
        m_surface = reinterpret_cast<VkSurfaceKHR>(nativeSurfaceHandle);
        // Swapchain-Erzeugung aus m_surface folgt in Phase 8 (Views).
        return {};
    }
