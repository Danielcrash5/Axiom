...
namespace axiom::renderer::rhi::vulkan {
    ...
    // Stelle, damit nichts (wie zuletzt
    // toVkDescriptorType/toVkShaderStageFlags) vergessen wird, weil es in einem
    // separaten Chat-Schnipsel stand.
    // ============================================================================
    namespace {
        ...
        VkFilter toVkFilter(FilterMode mode) {
            return mode == FilterMode::Linear ? VK_FILTER_LINEAR
                                              : VK_FILTER_NEAREST;
        }
    ...
    RHIResult<std::unique_ptr<VulkanBackend>> VulkanBackend::create(
        ...
        if (vkCreateInstance(&instanceInfo, nullptr, &backend->m_instance) !=
            VK_SUCCESS) {
        ...
        auto createDebugMessenger =
            reinterpret_cast<PFN_vkCreateDebugUtilsMessengerEXT>(
                vkGetInstanceProcAddr(backend->m_instance,
        ...
        if (createDebugMessenger) {
            ...
            createDebugMessenger(backend->m_instance, &debugInfo, nullptr,
                                 &backend->m_debugMessenger);
        }
#endif
        ...
        vkEnumeratePhysicalDevices(backend->m_instance, &deviceCount,
        ...
        VkDeviceCreateInfo deviceInfo{.sType =
        ...
        if (vkCreateDevice(chosen, &deviceInfo, nullptr, &backend->m_device) !=
            VK_SUCCESS) {
            return std::unexpected(RHIError::DeviceRequestFailed);
        }
        vkGetDeviceQueue(backend->m_device, chosenGraphicsFamily, 0,
                         &backend->m_graphicsQueue);
        ...
        if (vmaCreateAllocator(&allocatorInfo, &backend->m_allocator) !=
            VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }
        ...
        if (vkCreateCommandPool(backend->m_device, &poolInfo, nullptr,
                                &backend->m_uploadCommandPool) != VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }
        ...
        if (vkAllocateCommandBuffers(backend->m_device, &cmdAllocInfo,
                                     &backend->m_uploadCommandBuffer) !=
            VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }
        ...
        if (vkCreateFence(backend->m_device, &fenceInfo, nullptr,
                          &backend->m_uploadFence) != VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }

        // --- Graph-Command-Infrastruktur (Phase 2) ---
        VkCommandPoolCreateInfo graphPoolInfo{
            .sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO};
        graphPoolInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
        graphPoolInfo.queueFamilyIndex = chosenGraphicsFamily;
        if (vkCreateCommandPool(backend->m_device, &graphPoolInfo, nullptr,
                                &backend->m_graphCommandPool) != VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }

        VkFenceCreateInfo graphFenceInfo{
            .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
        if (vkCreateFence(backend->m_device, &graphFenceInfo, nullptr,
                          &backend->m_graphFence) != VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }
    ...
    VulkanBackend::~VulkanBackend() {
        ...
        for (auto &slot : m_textures) {
            if (slot.alive) {
                ...
                // besitzen wir) - das zugehoerige VkImage NICHT, das gehoert
                // der Swapchain und wird unten bei vkDestroySwapchainKHR
                // mitzerstoert.
        ...
        // Surface). Die Image-Views wurden oben schon ueber m_textures
        // zerstoert.
        ...
        for (auto &slot : m_bindGroups) {
            if (slot.alive)
                vkDestroyDescriptorPool(m_device, slot.pool,
                                        nullptr); // zerstoert auch das Set
        }
    ...
    VulkanBackend::createBuffer(const BufferDesc &desc) {
        if (desc.sizeBytes == 0) {
    ...
    VulkanBackend::createTexture(const TextureDesc &desc) {
        if (desc.width == 0 || desc.height == 0) {
            return std::unexpected(RHIError::InvalidDescriptor);
        }

        VkImageCreateInfo imageInfo{.sType =
        ...
        VkImage image;
        VmaAllocation allocation;
        if (vmaCreateImage(m_allocator, &imageInfo, &allocInfo, &image,
                           &allocation, nullptr) != VK_SUCCESS) {
            return std::unexpected(RHIError::OutOfMemory);
        }
        ...
        VkImageView view;
        if (vkCreateImageView(m_device, &viewInfo, nullptr, &view) !=
            VK_SUCCESS) {
    ...
    RHIResult<void> VulkanBackend::ensureStagingCapacity(uint64_t sizeBytes) {
        ...
        if (vmaCreateBuffer(m_allocator, &bufferInfo, &allocInfo,
                            &m_stagingBuffer, &m_stagingAllocation,
    ...
    RHIResult<void> VulkanBackend::submitImmediate(
        ...
        if (vkQueueSubmit(m_graphicsQueue, 1, &submitInfo, m_uploadFence) !=
            VK_SUCCESS) {
            return std::unexpected(RHIError::Unknown);
        }
    ...
    RHIResult<void>
    VulkanBackend::uploadBufferData(BufferHandle handle, uint64_t offset,
    ...
    RHIResult<void>
    VulkanBackend::uploadTextureData(TextureHandle handle,
        ...
        return submitImmediate([&](VkCommandBuffer cmd) {
            ...
            toTransferDst.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1,
                                              0, 1};

            VkDependencyInfo depInfo{.sType =
            ...
            region.bufferRowLength =
                uploadDesc.bytesPerRow != 0
                    ? uploadDesc.bytesPerRow / bytesPerPixel(slot.format)
                    : 0;
            ...
            VkDependencyInfo depInfo2{.sType =
                                          VK_STRUCTURE_TYPE_DEPENDENCY_INFO};
    ...
    void VulkanBackend::submit(CommandList &cmd) {
        ...
        // Synchron/blockierend - genau deshalb braucht present() (siehe
        // Swapchain- Abschnitt oben) keine Wait-Semaphores: zum Zeitpunkt des
        // Aufrufs ist die GPU-Arbeit bereits abgeschlossen. Frame-Pacing mit
        // mehreren Fences "in flight" (Ueberlappung von CPU/GPU-Arbeit) ist
        // eine spaetere Performance-Optimierung, kein Korrektheitsproblem.
    ...
    RHIResult<SurfaceHandle>
    VulkanBackend::createSurface(void *nativeSurfaceHandle) {
        ...
        m_surfaces[index].surface =
            reinterpret_cast<VkSurfaceKHR>(nativeSurfaceHandle);
        m_surfaces[index].alive = true;

        // Swapchain-Erzeugung aus dieser Surface:
        // IRHIBackend::createSwapchain(). Jede zusaetzliche
        // ImGui-Viewport-Fenster-Surface durchlaeuft denselben Pfad.
        return SurfaceHandle{index, m_surfaces[index].generation};
    }

    void VulkanBackend::destroySurface(SurfaceHandle handle) {
        if (handle.index >= m_surfaces.size())
            return;
    ...
    RHIResult<void>
    VulkanBackend::recreateSwapchainInternal(SwapchainSlot &slot,
        ...
        if (!presentSupported) {
            ...
            // dedizierte Present-Queue-Suche ist eine spaetere Erweiterung,
            // falls das auf einer Zielplattform mal nicht zutrifft.
            return std::unexpected(RHIError::InvalidDescriptor);
        }
        ...
        if (caps.currentExtent.width != 0xFFFFFFFFu) {
            ...
            extent.width = std::clamp(requestedWidth, caps.minImageExtent.width,
                                      caps.maxImageExtent.width);
            extent.height =
                std::clamp(requestedHeight, caps.minImageExtent.height,
                           caps.maxImageExtent.height);
        }
        if (extent.width == 0 || extent.height == 0) {
            // Minimiertes Fenster o.ae. - Swapchain-Erzeugung mit Groesse 0 ist
            // ungueltig. Aufrufer soll es spaeter (nach Restore) erneut
            // versuchen.
            return std::unexpected(RHIError::InvalidDescriptor);
        }

        uint32_t formatCount = 0;
        vkGetPhysicalDeviceSurfaceFormatsKHR(m_physicalDevice, slot.surface,
                                             &formatCount, nullptr);
        if (formatCount == 0)
            return std::unexpected(RHIError::Unknown);
        ...
        // eine bewusste Vereinfachung fuer den ersten Wurf; auf den
        // allermeisten Desktop-Plattformen liefert obige Bevorzugung ohnehin
        // eines der beiden.
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(
            m_physicalDevice, slot.surface, &presentModeCount, nullptr);
        std::vector<VkPresentModeKHR> presentModes(presentModeCount);
        vkGetPhysicalDeviceSurfacePresentModesKHR(
            m_physicalDevice, slot.surface, &presentModeCount,
            presentModes.data());

        VkPresentModeKHR chosenMode =
            VK_PRESENT_MODE_FIFO_KHR; // immer garantiert verfuegbar
        if (!slot.vsync) {
        ...
        // Die alte Swapchain darf erst zerstoert werden, wenn keine Queue mehr
        // auf ihre Presentable Images zugreift. Fuer die aktuelle, bewusst
        // vollstaendig synchrone Implementierung warten wir deshalb hier einmal
        // auf das komplette Device, bevor die neue Swapchain erzeugt wird.
        ...
        // Alte Image-Views (ueber m_textures) und die alte Swapchain selbst
        // erst JETZT zerstoeren, nachdem die neue erfolgreich erzeugt wurde -
        // so schreibt Vulkan das per oldSwapchain-Retire-Pfad vor.
        ...
        vkGetSwapchainImagesKHR(m_device, slot.swapchain, &actualImageCount,
                                nullptr);
        ...
        for (VkImage image : images) {
            ...
            texSlot.image = image;
            texSlot.allocation =
                VK_NULL_HANDLE; // extern besessen - siehe externallyOwnedImage
            texSlot.view = view;
            ...
            slot.imageTextures.push_back(
                TextureHandle{index, texSlot.generation});
        }
    ...
    VulkanBackend::createSwapchain(const SwapchainDesc &desc) {
        ...
        VkFenceCreateInfo fenceInfo{.sType =
                                        VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
        if (vkCreateFence(m_device, &fenceInfo, nullptr, &slot.acquireFence) !=
            VK_SUCCESS) {
            slot.alive = false;
        ...
        if (auto result =
                recreateSwapchainInternal(slot, desc.width, desc.height);
            !result) {
            vkDestroyFence(m_device, slot.acquireFence, nullptr);
    ...
    VulkanBackend::acquireNextImage(SwapchainHandle handle) {
        if (handle.index >= m_swapchains.size()) {
            return std::unexpected(RHIError::InvalidDescriptor);
        }
        auto &slot = m_swapchains[handle.index];
        ...
        VkResult result = vkAcquireNextImageKHR(m_device, slot.swapchain,
                                                UINT64_MAX, VK_NULL_HANDLE,
                                                slot.acquireFence, &imageIndex);

        if (result == VK_ERROR_OUT_OF_DATE_KHR) {
            if (auto recreateResult =
                    recreateSwapchainInternal(slot, slot.width, slot.height);
                !recreateResult) {
                return std::unexpected(recreateResult.error());
            }
            result = vkAcquireNextImageKHR(m_device, slot.swapchain, UINT64_MAX,
                                           VK_NULL_HANDLE, slot.acquireFence,
                                           &imageIndex);
        }
    ...
    RHIResult<void> VulkanBackend::present(SwapchainHandle handle,
                                           uint32_t imageIndex) {
        if (handle.index >= m_swapchains.size()) {
            return std::unexpected(RHIError::InvalidDescriptor);
        }
        ...
        // Keine Wait-Semaphores noetig: submit() (VulkanBackend::submit)
        // blockiert bereits per Fence bis zur GPU-Fertigstellung, bevor es
        // zurueckkehrt - zum Zeitpunkt von present() ist das Rendering also
        // garantiert fertig.
    ...
    VulkanBackend::swapchainExtent(SwapchainHandle handle) const {
        if (handle.index >= m_swapchains.size())
            return {0, 0};
        auto &slot = m_swapchains[handle.index];
        if (!slot.alive || slot.generation != handle.generation)
    ...
    VulkanBackend::createPipeline(const PipelineDesc &desc) {
        VkShaderModule vertModule =
            createShaderModule(m_device, desc.shader.vertexSpirv);
        VkShaderModule fragModule =
            createShaderModule(m_device, desc.shader.pixelSpirv);

        if ((desc.shader.stages & ShaderStage::Vertex) &&
            vertModule == VK_NULL_HANDLE) {
            return std::unexpected(RHIError::InvalidDescriptor);
        }
        if ((desc.shader.stages & ShaderStage::Pixel) &&
        ...
        for (auto &attr : desc.shader.vertexLayout.attributes) {
            attributes.push_back({.location = attr.location,
                                  .binding = 0,
                                  .format = toVkVertexFormat(attr.format),
                                  .offset = attr.offset});
        }
        ...
        vertexInput.vertexBindingDescriptionCount =
            desc.shader.vertexLayout.stride > 0 ? 1 : 0;
        ...
        if (desc.state.blend != BlendMode::None) {
            ...
            blendAttachment.dstColorBlendFactor =
                desc.state.blend == BlendMode::Additive
                    ? VK_BLEND_FACTOR_ONE
        ...
        if (desc.depthTargetFormat) {
            renderingInfo.depthAttachmentFormat =
                toVkFormat(*desc.depthTargetFormat);
        }
    ...
    void VulkanBackend::destroyPipeline(PipelineHandle handle) {
        if (handle.index >= m_pipelines.size())
            return;
    ...
    VulkanBackend::createBindGroupLayout(const BindGroupLayoutDesc &desc) {
        ...
        for (auto &entry : desc.entries) {
            ...
            binding.descriptorCount =
                entry.bindless ? entry.bindlessMaxCount : 1;
        ...
        return BindGroupLayoutHandle{index,
                                     m_bindGroupLayouts[index].generation};
    }

    void VulkanBackend::destroyBindGroupLayout(BindGroupLayoutHandle handle) {
        if (handle.index >= m_bindGroupLayouts.size())
            return;
    ...
    VulkanBackend::createBindGroup(const BindGroupDesc &desc) {
        ...
        VkDescriptorSet set;
        if (vkAllocateDescriptorSets(m_device, &allocInfo, &set) !=
            VK_SUCCESS) {
        ...
        for (auto &entry : desc.entries) {
            if (entry.buffer) {
                bufferInfos.push_back(
                    {nativeBuffer(*entry.buffer), 0, VK_WHOLE_SIZE});
                writes.push_back(
                ...
                imageInfos.push_back(
                    {VK_NULL_HANDLE, nativeImageView(*entry.texture),
                ...
                imageInfos.push_back({nativeSampler(*entry.sampler),
                                      VK_NULL_HANDLE,
    ...
    void VulkanBackend::destroyBindGroup(BindGroupHandle handle) {
        if (handle.index >= m_bindGroups.size())
            return;
    ...
    RHIResult<void>
    VulkanBackend::updateBindGroup(BindGroupHandle target,
        ...
        if (!writes.empty()) {
            ...
            // das Set potenziell noch "in flight" gebunden ist (Phase 1
            // Device-Feature).
            vkUpdateDescriptorSets(m_device,
                                   static_cast<uint32_t>(writes.size()),
                                   writes.data(), 0, nullptr);
    ...
    RHIResult<SamplerHandle>
    VulkanBackend::createSampler(const SamplerDesc &desc) {
    ...
    std::pair<uint32_t, uint32_t>
    VulkanBackend::nativeExtent(TextureHandle handle) const {
        if (handle.index >= m_textures.size())
            return {0, 0};
        auto &slot = m_textures[handle.index];
        if (!slot.alive || slot.generation != handle.generation)
    ...
    VulkanBackend::NativePipeline
    VulkanBackend::nativePipeline(PipelineHandle handle) const {
        if (handle.index >= m_pipelines.size())
            return {};
        auto &slot = m_pipelines[handle.index];
        if (!slot.alive || slot.generation != handle.generation)
    ...
    VkDescriptorSetLayout VulkanBackend::nativeDescriptorSetLayout(
        BindGroupLayoutHandle handle) const {
        if (handle.index >= m_bindGroupLayouts.size())
            return VK_NULL_HANDLE;
        auto &slot = m_bindGroupLayouts[handle.index];
        if (!slot.alive || slot.generation != handle.generation)
    ...
    VkDescriptorSet
    VulkanBackend::nativeDescriptorSet(BindGroupHandle handle) const {
        if (handle.index >= m_bindGroups.size())
            return VK_NULL_HANDLE;
        auto &slot = m_bindGroups[handle.index];
        if (!slot.alive || slot.generation != handle.generation)
