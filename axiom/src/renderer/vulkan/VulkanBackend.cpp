...
namespace axiom::renderer::rhi::vulkan {
    ...
    RHIResult<std::unique_ptr<VulkanBackend>> VulkanBackend::create() {
        ...
        deviceInfo.pNext =
            &enableFeatures2; // Features2-Chain statt pEnabledFeatures
    ...
    // ensureStagingCapacity, submitImmediate, uploadBufferData,
    // uploadTextureData bleiben INHALTLICH IDENTISCH zur letzten Version – die
    // nutzten schon reines VkBuffer/VkImage/VmaAllocation, keine vkb-Typen.
    // Kannst du 1:1 aus meiner letzten Nachricht übernehmen.
    ...
    VulkanBackend::createPipeline(const PipelineDesc &desc) {
        ...
        rasterizer.frontFace =
            VK_FRONT_FACE_COUNTER_CLOCKWISE; // Standard bei rechtshändig
        ...
        for (auto handle : desc.bindGroupLayouts) {
            setLayouts.push_back(
                nativeDescriptorSetLayout(handle)); // Helper, siehe unten
        }
    ...
    VulkanBackend::createBindGroup(const BindGroupDesc &desc) {
        ...
        for (auto &entry : desc.entries) {
            ...
            // texture/sampler-Bindings folgen analog, hier gekürzt (gleiche
            // Fälle wie Bindless-Heap in Punkt 13, dort ausführlicher).
        }
