...
namespace axiom::renderer::rhi::vulkan {
    ...
    namespace {
        ...
        // Grobe, konservative Stage/Access-Masken pro Layout – reicht für Phase
        // 2 (Clear-Test). Feinere, pass-spezifische Barriers kommen mit Phase
        // 3, sobald Pipelines/Sampling-Kontext existieren.
    ...
    void VulkanCommandList::transitionTexture(TextureHandle texture,
        ...
        if (image == VK_NULL_HANDLE)
            return; // stale/ungültiges Handle, still ignorieren
        ...
        // Aspect-Mask hart auf COLOR – Depth-Texturen brauchen später (Phase
        // 3+) eine formatabhängige Unterscheidung, ist für den Clear-Test
        // irrelevant.
    ...
    void VulkanCommandList::clearTexture(TextureHandle texture,
                                         ClearColor color) {
        VkImage image = m_backend.nativeImage(texture);
        if (image == VK_NULL_HANDLE)
            return;
        ...
        // Erwartet TRANSFER_DST_OPTIMAL – RenderGraph::execute() stellt das
        // über requiredLayoutFor(AccessType::Write) + transitionTexture()
        // vorher sicher.
        vkCmdClearColorImage(m_commandBuffer, image,
                             VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearValue,
                             1, &range);
    }

    void VulkanCommandList::copyBufferToBuffer(BufferHandle src,
                                               uint64_t srcOffset,
                                               BufferHandle dst,
                                               uint64_t dstOffset,
                                               uint64_t sizeBytes) {
    ...
    void VulkanCommandList::bindPipeline(PipelineHandle handle) {
        auto [pipeline, layout] = m_backend.nativePipeline(handle);
        if (pipeline == VK_NULL_HANDLE)
            return;
        m_currentPipelineLayout = layout; // für bindGroup gebraucht
        vkCmdBindPipeline(m_commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipeline);
    }

    void VulkanCommandList::bindVertexBuffer(uint32_t slot,
                                             BufferHandle handle) {
    ...
    void VulkanCommandList::bindIndexBuffer(BufferHandle handle,
                                            bool use16Bit) {
        VkBuffer buffer = m_backend.nativeBuffer(handle);
        if (buffer == VK_NULL_HANDLE)
            return;
        vkCmdBindIndexBuffer(m_commandBuffer, buffer, 0,
                             use16Bit ? VK_INDEX_TYPE_UINT16
    ...
    void VulkanCommandList::beginRendering(
        TextureHandle colorTarget, std::optional<TextureHandle> depthTarget) {
        ...
        colorAttachment.loadOp =
            VK_ATTACHMENT_LOAD_OP_LOAD; // RenderGraph hat schon geclearet falls
                                        // nötig
        ...
        // Viewport/Scissor hier setzen (dynamic state), inkl. Y-Flip (Abschnitt
        // 1)
