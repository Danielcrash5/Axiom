...
void VulkanCommandList::transitionTexture(TextureHandle texture,
    ...
    if (image == VK_NULL_HANDLE) return; // stale/ungültiges Handle, still ignorieren
...
void VulkanCommandList::beginRendering(TextureHandle colorTarget, std::optional<TextureHandle> depthTarget) {
    ...
    colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD; // RenderGraph hat schon geclearet falls nötig
