...
// Grobe, konservative Stage/Access-Masken pro Layout – reicht für Phase 2
// (Clear-Test). Feinere, pass-spezifische Barriers kommen mit Phase 3,
// sobald Pipelines/Sampling-Kontext existieren.
void stageAccessForLayout(VkImageLayout layout, VkPipelineStageFlags2 &stage,
...
void VulkanCommandList::transitionTexture(TextureHandle texture,
    ...
    // Aspect-Mask hart auf COLOR – Depth-Texturen brauchen später (Phase 3+)
    // eine formatabhängige Unterscheidung, ist für den Clear-Test irrelevant.
...
void VulkanCommandList::clearTexture(TextureHandle texture, ClearColor color) {
    ...
    // Erwartet TRANSFER_DST_OPTIMAL – RenderGraph::execute() stellt das über
...
void VulkanCommandList::bindGroup(uint32_t set, BindGroupHandle handle) {
    ...
    vkCmdBindDescriptorSets(m_commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            m_currentPipelineLayout, set, 1, &descriptorSet, 0,
                            nullptr);
}
...
void VulkanCommandList::beginRendering(
    ...
    // Viewport/Scissor hier setzen (dynamic state), inkl. Y-Flip (Abschnitt 1)
