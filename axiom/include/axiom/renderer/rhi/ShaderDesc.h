...
namespace axiom::renderer::rhi {
    ...
    [[nodiscard]] constexpr ShaderStage operator|(ShaderStage a,
                                                  ShaderStage b) noexcept {
        return static_cast<ShaderStage>(static_cast<uint32_t>(a) |
                                        static_cast<uint32_t>(b));
    }
    [[nodiscard]] constexpr bool operator&(ShaderStage a,
                                           ShaderStage b) noexcept {
        return (static_cast<uint32_t>(a) & static_cast<uint32_t>(b)) != 0;
    }

    // SPIR-V ist 32-bit-wortweise aligned, daher span<const uint32_t> statt
    // Bytes.
    struct ShaderDesc {
