...
namespace axiom::renderer::rhi::vulkan {
    ...
    class VulkanBackend final : public IRHIBackend {
      public:
        ...
        RHIResult<void>
        uploadBufferData(BufferHandle, uint64_t offset,
                         std::span<const std::byte> data) override;
        RHIResult<void>
        uploadTextureData(TextureHandle, const TextureUploadDesc &,
                          std::span<const std::byte> data) override;

        std::unique_ptr<CommandList> createCommandList() override;
        void submit(CommandList &) override;

        [[nodiscard]] VkImage nativeImage(rhi::TextureHandle handle) const;
        [[nodiscard]] VkBuffer nativeBuffer(rhi::BufferHandle handle) const;
