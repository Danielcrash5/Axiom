...
namespace axiom {
    ...
    class VFS {
      public:
        enum class MountType { Directory, Zip };
        ...
        static bool ReadTextFile(const std::string &virtualPath,
                                 std::string &outText);

        static bool WriteFile(const std::string &virtualPath,
                              const std::vector<uint8_t> &data);
        static bool WriteTextFile(const std::string &virtualPath,
        ...
        GetLastWriteTime(const std::string &virtualPath);

        static std::vector<std::string>
        ListFiles(const std::string &virtualPath, bool recursive = false);
        static std::vector<std::string>
        ListDirectories(const std::string &virtualPath, bool recursive = false);
      ...
      private:
        ...
        static bool ResolvePath(const std::string &virtualPath,
                                ResolvedPath &out);
