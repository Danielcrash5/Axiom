...
namespace axiom {
    ...
    class UUID {
      public:
        ...
        // zwischen Domaenen (Asset/Entity/etc.), siehe TypedUUID<Tag>.
...
namespace std {
    template <> struct hash<axiom::UUID> {
        std::size_t operator()(const axiom::UUID& uuid) const {
            // Boost-Style hash_combine -- reines XOR wuerde die
            // Verteilungsqualitaet in unordered_map verschlechtern.
...
namespace axiom {
    ...
    // 128-Bit, portabel als zwei uint64_t statt __int128
    // (kein Standard-C++, MSVC unterstuetzt es nicht, std::hash kennt es
    // nicht).
    class UUID {
      public:
        UUID(); // erzeugt neue, zufaellige 128-Bit-UUID
        explicit UUID(uint64_t hi,
                      uint64_t lo); // KEINE implizite Konversion mehr
        explicit UUID(
            const std::string &hex32); // aus 32-stelligem Hex-String parsen
