...
void ImGui::ImageWithBg(ImTextureRef tex_ref, const ImVec2 &image_size,
    ...
    if (g.Style.ImageBorderSize > 0.0f)
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border),
                                  rounding, g.Style.ImageBorderSize);
}
...
bool ImGui::DataTypeApplyFromText(const char *buf, ImGuiDataType data_type,
    ...
    if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) {
        ...
        if (format[0] == '\0')
            format = type_info->ScanFmt; // Format doesn't want us to show the
                                         // number currently, but we still need
                                         // to parse the resulting input
    }
...
bool ImGui::TempInputText(const ImRect &bb, ImGuiID id, const char *label,
    ...
    g.LastItemData.ItemFlags |=
        ImGuiItemFlags_AllowDuplicateId; // Using ImGuiInputTextFlags_MergedItem
                                         // above will skip ItemAdd() so we poke
                                         // here.
    bool value_changed =
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
    ...
    bool input_edited =
        ...
        0; // We would be using 'ret' if ImGuiInputTextFlags_EnterReturnsTrue
           // was not involved.
    bool value_changed =
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
    ...
    if (is_multiline) {
        ...
        bool no_clip =
            ...
            (id == g.NavActivateId); // Mimic some of ItemAdd() logic + add
    ...
    if (is_wordwrap)
        wrap_width =
            ImMax(1.0f, GetContentRegionAvail().x +
                            (draw_window->ScrollbarY ? 0.0f
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow)) {
            ...
            if (const char *clipboard = GetClipboardText()) {
                ...
                if (clipboard_filtered.Size > 0) // If everything was filtered,
    ...
    if (g.ActiveId == id) {
        ...
        if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
            ...
            if (event_flag) {
                ...
                state->Stb->select_start = ImClamp(callback_data.SelectionStart,
                                                   0, callback_data.BufTextLen);
                ...
                if (callback_data.BufDirty) {
                    ...
                    InputTextReconcileUndoState(
                        ...
                        callback_data.BufTextLen);
                    state->TextLen =
                        callback_data.BufTextLen; // Assume correct length and
    ...
    if (render_cursor || render_selection) {
        ...
        if (new_scroll_y != scroll_y) {
            ...
            draw_pos.y += (draw_window->Scroll.y -
                           scroll_y); // Manipulate cursor pos immediately avoid
                                      // a frame of lag
        ...
        if (render_selection) {
            ...
            const float bg_offy_up =
                is_multiline ? 0.0f
                             : -1.0f; // FIXME-DPI: those offsets should be part
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(
                ...
                1.0f * (float)(int)g.Style
                           ._MainScale); // Color buttons are often in need of
                                         // some sort of border // FIXME-DPI
    }
...
void ImGui::TreeNodeDrawLineToChildNode(const ImVec2 &target_pos) {
    ...
    if (rounding > 0.0f) {
        ...
        window->DrawList->AddLineH(x1, x2, y, GetColorU32(ImGuiCol_TreeLines),
                                   g.Style.TreeLinesSize);
    }
}
    ...
    if (flags & ImGuiMultiSelectFlags_ScopeRect)
        window->DC.CursorMaxPos =
            ms->ScopeRectMin; // CalcScopeRect() for
                              // ImGuiMultiSelectFlags_ScopeRect will measure in
                              // EndMultiSelect().
            ...
            if (!window->DC
                ...
                if (ImGuiTable *table = g.CurrentTable)
                    if (table->CurrentColumn != -1) {
                        ...
                        if (clip_min_x !=
                            clip_max_x) // When zero sized we expect that bounds
                                        // have been clamped and thus are
                                        // unreliable
                        {
                            item_rect.Min.x =
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags,
    ...
    IM_ASSERT(
        ...
        tab_bar); // Can't work while the tab bar is active as our tab doesn't
                  ...
                  // but we'd need to solve why triggers the commented early-out
