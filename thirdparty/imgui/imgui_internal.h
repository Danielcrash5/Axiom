...
// Enable SSE intrinsics if available
    ...
    !defined(IMGUI_DISABLE_SSE) && !defined(_M_ARM64) && !defined(_M_ARM64EC)
...
#pragma GCC diagnostic ignored                                                 \
    "-Wdeprecated-enum-enum-conversion" // warning: bitwise operation between
                                        ...
                                        // is deprecated
#pragma GCC diagnostic ignored                                                 \
    "-Wsign-conversion" // warning: conversion to 'xxxx' from 'xxxx' may change
...
template <typename T> T ImLerp(double a, double b, float t) {
...
template <typename T, int BLOCKSIZE> struct ImStableVector {
    int Size = 0;
    int Capacity = 0;
    ImVector<T *> Blocks;
...
struct IMGUI_API ImDrawListSharedData {
    ...
    float CurveTessellationTol;       // Tessellation tolerance when using
                                      // PathBezierCurveTo()
    float CircleTessellationMaxError; // Number of circle segments to use per
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    ImGuiButtonFlags_PressedOnRelease =
        1 << 7, // return true on release (default requires click+release).
                // Prior to 2026/03/20 this implied
                // ImGuiButtonFlags_NoHoldingActiveId but they are separate now.
...
struct IMGUI_API ImGuiInputTextState {
    ...
    bool
        CursorCenterY; // set when we want scrolling to be centered over the
...
struct ImGuiViewportP : public ImGuiViewport {
    ...
    float BgFgDrawListsLastTimeActive[2]; // Last frame number the background
                                          // (0) and foreground (1) draw lists
                                          // were used
    ImDrawList
...
struct ImGuiContext {
    ...
    float
        FontBakedScale; // == FontBaked->Size / FontSize. Scale factor over
                        // baked size. Rarely used nowadays, very often == 1.0f.
    ...
    ImRect NavScoringRect; // Rectangle used for scoring, in screen space. Based
                           ...
                           // navigation scoring.
    ImRect NavScoringNoClipRect; // Some nav operations (such as
    ...
    // platforms.
    bool ConfigNavEnableTabbing; // = true. Enable tabbing (Tab, Shift+Tab).
    ...
    ImGuiID
        InputTextReactivateId; // ID of InputText to reactivate on next frame
                               // (for io.ConfigInputTextEnterKeepActive
                               // behavior)
...
struct IMGUI_API ImGuiWindow {
    ...
    ImGuiWindowBgClickFlags BgClickFlags
        : 8; // Configure behavior of click+dragging on window bg/void or over
             // items. Default sets by io.ConfigWindowsMoveFromTitleBarOnly. If
             // you use this please report in #3379.
    ...
    ImGuiWindow *
        ParentWindowForFocusRoute; // Set to manual link a window to its logical
                                   ...
                                   // honored (e.g. Tool linked to Document)
...
struct IMGUI_API ImGuiTable {
    ...
    ImGuiTableColumnIdx LastHeldHeaderColumn; // Index of column header being
                                              // held from previous frame.
    ImGuiTableColumnIdx
        ReorderColumn; // Index of column being reordered. (not cleared)
    ImGuiTableColumnIdx ReorderColumnDstOrder; // Requested display order of
...
} // FIXME-DPI: I don't want to formalize this just yet. Because reasons. Please
  // don't use.
...
IMGUI_API void ExtendHitBoxWhenNearViewportEdge(ImGuiWindow *window, ImRect *bb,
                                                float threshold,
                                                ImGuiAxis axis);
...
} // This may be useful to apply workaround that a based on distinguish whenever
  // an item is active as a text input field.
...
struct ImFontAtlasRectEntry {
    int TargetIndex
        : 20; // When Used: ImFontAtlasRectId -> into Rects[]. When unused:
              // index to next unused RectsIndex[] slot to consume free-list.
...
IMGUI_API void ImTextureDataQueueUpload(ImTextureData *tex, int x, int y, int w,
                                        int h);
