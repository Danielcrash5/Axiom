...
#define IMGUI_HAS_TEXTURES // Added ImGuiBackendFlags_RendererHasTextures - from
                           // IMGUI_VERSION_NUM >= 19198
...
struct ImTextureRef {
    ...
    inline ImTextureID
    GetTexID() const; // == (_TexData ? _TexData->TexID : _TexID) // Implemented
                      // below in the file.
    ...
    ImTextureID _TexID; // _OR_ Low-level backend texture identifier, if already
                        // uploaded or created by user/app. Generally provided
                        // to e.g. ImGui::Image() calls.
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
GetDrawData(); // valid after Render() and until the next call to NewFrame().
               // Call ImGui_ImplXXXX_RenderDrawData() function in your Renderer
               // Backend to render.
...
IMGUI_API void TextUnformatted(
    ...
    const char *text_end =
        NULL); // raw text without formatting. Practically equivalent to
               // 'Text("%s", text)' but doesn't require null terminated string
               // if 'text_end' is specified.
...
SetNextItemAllowOverlap(); // allow next item to be overlapped by a subsequent
                           ...
                           // Selectable(), TreeNode() covering an area where
                           // subsequent items may need to be added. Note that
                           // both Selectable() and TreeNode() have dedicated
                           // flags doing this.
...
IMGUI_API ImDrawList *GetBackgroundDrawList(
    ImGuiViewport *viewport =
        NULL); // get background draw list for the given viewport or viewport
               ...
               // first rendering one. Useful to quickly draw shapes/text behind
               // dear imgui contents.
IMGUI_API ImDrawList *GetForegroundDrawList(
    ImGuiViewport *viewport =
        NULL); // get foreground draw list for the given viewport or viewport
               // associated to the current window. this draw list will be the
               // top-most rendered one. Useful to quickly draw shapes/text over
               // dear imgui contents.
...
IMGUI_API bool SetItemKeyOwner(
    ImGuiKey
        key); // Set key owner to last item ID if it is hovered or active.
              // Return true when ownership has been set. Roughly equivalent to
              // 'if (TestKeyOwner(key, GetItemID()) && (IsItemHovered() ||
...
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool
        want_capture_mouse); // Override io.WantCaptureMouse flag next frame
                             ...
                             // handle, typical when true it instructs your app
...
IMGUI_API ImGuiViewport *
FindViewportByID(ImGuiID viewport_id); // this is a helper for backends.
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_CtrlEnterForNewLine =
        1 << 8, // In multi-line mode: validate with Enter, add new line with
                ...
                // add line with Enter). Note that Shift+Enter always enter a
                // new line either way.
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_AllowOverlap =
        1 << 2, // Hit testing will allow subsequent widgets to overlap this
                // one. Require previous frame HoveredId to match before being
                // usable. Shortcut to calling SetNextItemAllowOverlap().
    ...
    ImGuiTreeNodeFlags_Leaf =
        ...
        << 8, // No collapsing, no arrow (use as a convenience for leaf nodes).
              ...
              // never use that scope, add ImGuiTreeNodeFlags_NoTreePushOnOpen.
...
enum ImGuiSelectableFlags_ {
    ...
    ImGuiSelectableFlags_AllowOverlap =
        1 << 4, // Hit testing will allow subsequent widgets to overlap this
                // one. Require previous frame HoveredId to match before being
                // usable. Shortcut to calling SetNextItemAllowOverlap().
...
enum ImGuiKey : int {
    ...
    ImGuiKey_GamepadFaceLeft, // X           | Y       | Square   | Toggle Menu.
                              // Hold for Windowing mode (Focus/Move/Resize
                              // windows)
    ImGuiKey_GamepadFaceRight, // B           | A       | Circle   | Cancel /
                               // Close / Exit
    ImGuiKey_GamepadFaceUp,   // Y           | X       | Triangle | Open Context
                              // Menu
    ImGuiKey_GamepadFaceDown, // A           | B       | Cross    | Activate /
                              // Open / Toggle. Hold for 0.60f to Activate in
                              // Text Input mode (e.g. wired to an on-screen
                              // keyboard).
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableKeyboard =
        1 << 0, // Master keyboard navigation enable flag. Enable full Tabbing +
                // directional arrows + Space/Enter to activate. Note: some
                // features such as basic Tabbing and CtrL+Tab are enabled by
                // regardless of this flag (and may be disabled via other means,
                // see #4828, #9218).
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseHoveredViewport =
        1 << 12, // Backend Platform supports calling io.AddMouseViewportEvent()
    ...
    ImGuiBackendFlags_HasParentViewport =
        1 << 13, // Backend Platform supports honoring
                 // viewport->ParentViewport/ParentViewportId value, by applying
                 // the corresponding parent/child relationship at the Platform
                 // level. Child windows always appear in front of their parent
                 // window.
};
...
enum ImGuiCol_ {
    ...
    ImGuiCol_CheckboxSelectedBg, // Checkbox background when Selected, otherwise
                                 // use FrameBg
    ImGuiCol_SliderGrab,
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_AllowOverlap =
        1 << 12, // Hit testing will allow subsequent widgets to overlap this
                 // one. Require previous frame HoveredId to match before being
                 // usable. Shortcut to calling SetNextItemAllowOverlap().
};
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoDragDrop =
        1
        << 9, //              // ColorEdit: disable drag and drop target/source.
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_Reorderable =
        1 << 1, // Enable reordering columns in header row. (Need calling
                // TableSetupColumn() + TableHeadersRow() to display headers, or
                // using ImGuiTableFlags_ContextMenuInBody to access
                // context-menu without headers).
    ...
    ImGuiTableFlags_ContextMenuInBody =
        1 << 5, // Right-click on columns body/contents will also display table
...
struct ImGuiStyle {
    ...
    IMGUI_API void ScaleAllSizes(
        float scale_factor); // Scale all spacing/padding/thickness values. Do
                             // not scale fonts. See comments in definition.
                             // Consider not calling this if your initial scale
                             // factor if <1.0.
...
struct ImGuiIO {
    ...
    bool ConfigDockingNoDockingOver; // = false          // Simplified docking
                                     // mode: disable window merging into a same
                                     // tab-bar, so docking is limited to
                                     // splitting windows.
    ...
    bool ConfigViewportsNoDefaultParent; // = true           // Disable setting
                                         // OS window parent to main viewport by
                                         // default. The platform backend is
                                         // expected to honor
                                         // `viewport->ParentViewportID` to
                                         // setup a parent/child relationship
                                         // between the OS windows (supported if
                                         // ImGuiBackendFlags_HasParentViewport
                                         // is set). When parented: child
                                         // windows always appear in front of
                                         // their parent. Set to false if you
                                         ...
                                         // may be set on a per-window basis
    ...
    bool
        ConfigViewportsPlatformFocusSetsImGuiFocus; //= true // When a platform
                                                    ...
                                                    //Platform Title Bar), apply
                                                    //corresponding focus on
                                                    //imgui windows (may clear
                                                    //focus/active id from imgui
                                                    //windows location in other
                                                    //platform windows). In
                                                    //principle this is better
                                                    //enabled but we provide an
                                                    //opt-out, because some
                                                    //Linux window managers tend
                                                    //to eagerly focus windows
                                                    //(e.g. on mouse hover, or
                                                    //even a simple window
                                                    //pos/size change).

    // DPI/Scaling options
    // This may keep evolving during 1.92.x releases. Expect some turbulence.
    bool ConfigDpiScaleFonts; // = false          // [EXPERIMENTAL]
                              // Automatically overwrite style.FontScaleDpi when
                              // Monitor DPI changes. This will scale fonts but
    ...
    bool ConfigInputTextEnterKeepActive; // = false          // [BETA] Pressing
                                         // Enter will reactivate item and
                                         // select all text (single-line only).
    ...
    AddKeyAnalogEvent(ImGuiKey key, bool down,
                      float v); // Queue a new key down/up event for analog
                                // values (e.g. ImGuiKey_Gamepad_ values).
    ...
    SetKeyEventNativeData(ImGuiKey key, int native_keycode, int native_scancode,
                          int native_legacy_index =
                              -1); // [Optional] Specify index for legacy <1.87
                                   // IsKeyXXX() functions with native indices +
    ...
    IMGUI_API void SetAppAcceptingEvents(
        bool accepting_events); // Set master flag for accepting key/mouse/text
                                ...
                                // to disable events being queued while your app
                                // is frozen.
...
struct ImGuiWindowClass {
    ...
    void *PlatformIconData;     // [EXPERIMENTAL] Pass opaque data for Platform
                                // backend to handle.
...
struct ImGuiListClipper {
    ...
    int UserIndex;    // Helper storage for user convenience/code. Optional, and
                      // otherwise unused if you don't use it.
...
enum ImGuiMultiSelectFlags_ {
    ...
    ImGuiMultiSelectFlags_BoxSelectNoScroll =
        1 << 8, // Disable scrolling when box-selecting and moving mouse near
                // edges of scope.
    ...
    ImGuiMultiSelectFlags_SelectOnAuto =
        1 << 13, // Apply selection on mouse down when clicking on unselected
                 // item, on mouse up when clicking on selected item. (Default)
    ImGuiMultiSelectFlags_SelectOnClickAlways =
        1
        << 14, // Apply selection on mouse down when clicking on any items.
               // Prevents Drag and Drop from being used on multiple-selection,
               // but allows e.g. BoxSelect to always reselect even when
               // clicking inside an existing selection. (Excel style behavior)
    ImGuiMultiSelectFlags_SelectOnClickRelease =
        1 << 15, // Apply selection on mouse release when clicking an unselected
...
struct ImGuiSelectionExternalStorage {
    ...
    void (*AdapterSetItemSelected)(
        ...
        bool
            selected); // e.g. AdapterSetItemSelected =
...
enum ImDrawFlags_ {
    ...
    ImDrawFlags_RoundCornersTopLeft =
        1 << 4, // Round top-left corner only (when rounding > 0.0f, we default
                // to all corners).
    ImDrawFlags_RoundCornersTopRight =
        1 << 5, // Round top-right corner only (when rounding > 0.0f, we default
                // to all corners).
    ImDrawFlags_RoundCornersBottomLeft =
        1 << 6, // Round bottom-left corner only (when rounding > 0.0f, we
                // default to all corners).
    ImDrawFlags_RoundCornersBottomRight =
        1 << 7, // Round bottom-right corner only (when rounding > 0.0f, we
                // default to all corners).
    ImDrawFlags_RoundCornersNone =
        1 << 8, // Disable rounding even if `float rounding > 0.0f`. This is NOT
                // zero, NOT an implicit flag!
    ...
    // Stroke options
    ImDrawFlags_Closed = 1 << 9, // PathStroke(), AddPolyline(): specify that
                                 // shape should be closed.
    // ImDrawFlags_Closed                    = 1 << 0, // Prior to 1.92.8 (May
...
enum ImDrawListFlags_ {
    ...
    ImDrawListFlags_TextNoPixelSnap =
        1 << 4, // Disable automatically snapping AddText() calls to pixel
                // boundaries.
};
...
struct ImDrawList {
    ...
    } // OBSOLETED in 1.92.8: NEW FUNCTION SIGNATURE HAS 'thickness' AND 'flags'
      // SWAPPED.
...
struct ImFontConfig {
    ...
    bool FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken by
                               ...
                               // itself). SINCE 1.92, THE DATA NEEDS TO PERSIST
                               // FOR WHOLE DURATION OF ATLAS.
    ...
    float GlyphExtraAdvanceX; // 0        // Extra spacing (in pixels) between
                              // glyphs. Please contact us if you are using
                              // this. // FIXME-NEWATLAS: Intentionally unscaled
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFont *
    AddFontDefault(const ImFontConfig *font_cfg =
    ...
    IMGUI_API ImFont *
    AddFontDefaultBitmap(const ImFontConfig *font_cfg =
                             NULL); // Embedded classic pixel-clean font.
    ...
    IMGUI_API void
    ClearFonts(); // Clear input+output font data/glyphs. New fonts and textures
                  // will be recreated afterwards.
    IMGUI_API void
    ...
    IMGUI_API void
    RemoveCustomRect(ImFontAtlasRectId id); // Unregister a rectangle. Existing
                                            // pixels will stay in texture until
...
struct ImFontBaked {
    ...
    unsigned int MetricsTotalSurface
        : 26; // 3  // out // Total surface in pixels to get an idea of the font
              // rasterization/texture cost (not exact, we approximate the cost
              // of padding between glyphs)
    ...
    unsigned int LoadNoRenderOnLayout
        : 1; // 0  //     // Enable a two-steps mode where CalcTextSize() calls
             ...
             // advantageous if you know that the glyph is unlikely to actually
...
struct ImFont {
    ...
    ImWchar EllipsisChar; // 2-4   // out // Character used for ellipsis
                          // rendering ('...'). If you ever want to temporarily
                          // swap this for an alternative/dummy char, make sure
                          // to clear EllipsisAutoBake.
    ...
    ImU8 Used8kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 8192 /
                        8]; // 1 bytes if ImWchar=ImWchar16, 17 bytes if
                            ...
                            // 8K codepoints that has one active glyph. This is
                            // mainly used to facilitate iterations across all
                            // used codepoints.
    bool EllipsisAutoBake;  // 1     //     // Mark when the "..." glyph (==
                            // EllipsisChar) needs to be generated by combining
                            // multiple '.'.
    ...
    AddRemapChar(ImWchar from_codepoint,
                 ImWchar to_codepoint); // Makes 'from_codepoint' character
                                        // points to 'to_codepoint' glyph.
...
struct ImGuiViewport {
    ...
    void *PlatformUserData; // void* to hold custom data structure for the OS /
                            ...
                            // function.
    void *PlatformIconData; // void* to hold custom data structure for the OS /
                            // platform to specify an icon. Currently unused for
                            // exposed to allow experiments.
...
struct ImGuiPlatformIO {
    ...
    ImDrawCallback DrawCallback_ResetRenderState; // Request to reset the
                                                  // graphics/render state.
    ImDrawCallback DrawCallback_SetSamplerLinear; // Request backend to set
                                                  // texture sampling to Linear.
    ImDrawCallback
    ...
    void (*Platform_ShowWindow)(
        ImGuiViewport *vp); // . . U . .  // Newly created windows are initially
                            // hidden so SetWindowPos/Size/Title can be called
                            // on them before showing the window
    ...
    ImVec2 (*Platform_GetWindowFramebufferScale)(
        ImGuiViewport *vp); // N . . . .  // Return viewport density. Always 1,1
                            // on Windows, often 2,2 on Retina display on
    ...
    int (*Platform_CreateVkSurface)(
        ...
        ImU64 *out_vk_surface); // (Optional) For a Vulkan Renderer to call into
                                // Platform code (since the surface creation
...
IMGUI_API void
Image(ImTextureRef tex_ref, const ImVec2 &image_size, const ImVec2 &uv0,
