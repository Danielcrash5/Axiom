...
static int TableGetMaxDisplayOrderAllowed(ImGuiTable *table, int src_order,
    ...
    if (table->FreezeColumnsRequest > 0)
        dst_order = (src_order < table->FreezeColumnsRequest)
...
void ImGui::EndTable() {
    ...
    const float inner_content_max_y = ImCeil(
        table
            ->RowPosY2); // Rounding final position is important as we currently
...
void ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags,
    ...
    IM_ASSERT_USER_ERROR_RET(table->IsLayoutLocked == false,
                             ...
                             "row!"); // Table layout is locked when submitting
                                      // a row or when calling
                                      // BeginMultiSelect() with box-select.
...
void ImGui::TableDrawBorders(ImGuiTable *table) {
    ...
    if ((table->Flags & ImGuiTableFlags_BordersInnerH) &&
        ...
        if (border_y >= table->BgClipRect.Min.y &&
            ...
            inner_drawlist->AddLineH(table->BorderX1, table->BorderX2, border_y,
...
static bool MenuItemForColumnReorder(const char *label, bool selected,
    ...
    float min_w = offsets->DeclColumns(0.0f, label_size.x, 0.0f,
    ...
    BeginDisabled(!enabled); // Don't use ImGuiSelectableFlags_Disabled so that
                             // Check mark is also affected.
    bool ret = Selectable(label, false, selectable_flags,
            ...
            if (IsItemActive() && IsMouseDragging(0) &&
                ...
                g.ActiveIdHasBeenEditedBefore =
                    true; // Disable toggle in MenuItemForColumnReorder() +
                          // start dimming to display allowed reorder targets.
                table->ReorderColumn = (ImGuiTableColumnIdx)column_n;
                if (!IsItemHovered()) {
                    int reorder_dir =
                    ...
                    int dst_order =
                        ...
                        (int)ImCeil(reorder_amount) *
                            reorder_dir; // Estimated target order, will be
